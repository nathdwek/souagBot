...
float farrayMax(float array[],int arrayLength){
   int i;
   float max = array[0];
   for (i=1;i<arrayLength;i++){
      if (array[i]*array[i] > max*max){
         max = array[i];
      }
   }
   return max;
...
int main(void){
   initFilter();
   int wave[10000];
   generateSineInput(10000,900,wave);
   float result900[10000];
   float result1100[10000];

   int i;
   float filterOutput[2];
   for (i=0;i<10000;i++){
      filterNewSample(wave[i], filterOutput);
      result900[i] = filterOutput[0];
      result1100[i] = filterOutput[1];
   }

   FILE *filePtr;
   filePtr = fopen("floatArray","w");
   for (i = 0; i < 4000; i++) {
      fprintf(filePtr, "%d\t%.5G\t%.5g\n",i, result1100[i], result900[i]);
   }

   printf("%.5G\n%.5G\n",rms(result900,10000),rms(result1100,10000));
}
